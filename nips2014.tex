...
\label{sec:perExemplar}
	...
	To achieve the goal we will use relaxed form per-exemplar SVM formulation. The SVM convex objective can be formulated as follows
	...
	\begin{align}
  	...
  	The vector $\Psi_j$ can be interpreted as a direction that separates the training thata in an optimal way.
...
\section{Why does exemplar-svm works}
  	The convex objective \eqref{eq:obj} consist of regularization term a two penalty terms with weights $C_1$ and $C_2$. We will show that if $C_2 \rightarrow 0$ the $\Psi_j \rightarrow \Phi^+_j$.

  	Lets assume for the moment that the weight of the negative data penalty $C_2 \rightarrow 0$. Then the regularization term and the first penalty term dominates the equation \eqref{eq:obj}. Since $\mathcal P_j$ contain only a single positive training data, the sum from eq. \eqref{eq:obj} becomes
