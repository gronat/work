...
\label{sec:perExemplar}
	...
	Hence, we aim on replacing the original database Fisher vector $\Phi$ with some new vector $\Psi$ that performs better in the sense of separation from descriptors of other places and that has a unit $L2$-norm in order to measure the similarity by a dot-product (eq. \eqref{eq:class}).


	\paragraph{Per-exemplar SVM learning.}  
	...
	\paragraph{Analysis of per-exemplar SVM objective.}  
		In the following will shown that when $C_2$ in equation~(\ref{eq:obj}) is close to zero the new descriptor $\Psi_j$ is identical to original $\Phi^+$, when $C_2$ increases $\Psi_j$ declines from $\Phi^+$ such that the measured distance between $\Psi_j$ and negative examples increases.
		...
		This objective is being minimized in $w_j$ and $b_j$. First, let us decompose $w_j$ into parallel and orthogonal part w.r.t. $\Phi^+$ and then let $C_2\rightarrow 0$ and analyze the behavior of the convex objective. After decomposition of $w_j$ such that $w_j=w_j^{\perp}+w_j^{||}$ the convex objective can be re-written as follows
	  	...
	  	Notice that in the second term the orthogonal part does not change the value of a dot-product since $(w_j^{\perp}+w_j^{||})\Phi^+=w_j^{||}\Phi^+$. Because $C_2 \rightarrow 0$ the first two terms dominate the equation \eqref{eq:decomposed}, hence in the limit case it follows that
	...
	\paragraph{Interpretation, re-normalization.}
